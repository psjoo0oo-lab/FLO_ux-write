...
- **영문 번역체 지양**: 최대한 영문 번역체를 사용하지 않도록 유의한다.
...
**이모지**:
- **프로덕트 UI에서는 원칙적으로 사용하지 않는다.**
- 극히 예외적인 경우(온보딩 환영, 특별한 축하 메시지 등)에만 문장 앞/뒤에 **하나만** 허용한다.
- 버튼, 타이틀, 토스트, 툴팁, 오류 메시지에는 **절대 사용하지 않는다.**
- 오류·장애 등 부정적 상황에는 **절대 사용하지 않는다.**
...
UI 요소의 성격에 따라 아래 규칙을 적용하세요.

### 4-1. 타이틀

해요체를 기본으로, 명사형이나 청유형을 허용합니다.

조사와 마침표는 생략합니다.

사용자가 '지금 무엇을 해야 하는지' 또는 '어떤 상태인지' 한눈에 알 수 있어야 합니다.

| ⭕️ 권장 (O) | ❌ 지양 (X) |
|------------|------------|
| 로그아웃할까요? | 로그아웃을 하시겠습니까? |
| 이용권 구매 완료 | 이용권을 구매했어요<br>이용권 구매가 완료되었습니다. |
...
사용자 행동에 대한 상태나 피드백을 전달합니다.

단순 통보보다 행동의 결과를 능동태로 말해주는 것을 권장합니다.
...
기본적으로 해요체로 쓰되, 간결성을 위해 주어를 생략하거나 명사로 끝내는 것을 허용합니다.

명령형이나 질문형보다는 정보 제공형 어투가 어울립니다. 친절한 어휘를 선택하되 문장이 길어지는 미사여구나 수식어를 넣지 않습니다.

| ⭕️ 권장 (O) | ❌ 지양 (X) |
|------------|------------|
| 이 곡과 비슷한 음악을 추천해요 | 이 기능은 사용자가 좋아할 만한 음악을 찾아드립니다 |
| 좋아요 표시한 곡 모음 | |

### 4-5. 플레이스홀더

사용자가 입력해야 할 데이터의 형식이나 예시를 보여줍니다.

**예외**: 입력 필드 밖에 별도의 레이블(제목)이 없는 경우에만 지시문을 사용합니다.

| ⭕️ 권장 (O) | ❌ 지양 (X) |
|------------|------------|
| 예: 재즈, 발라드, 힙합 | 입력 |
| 검색어를 입력하세요 | 여기에 입력하세요! |

### 4-6. 오류 메시지

원인 설명 + 해결책을 제공합니다.

첫 문장은 "~했어요/~하지 않아요"처럼 상황을 설명하고, 두 번째 문장은 "~해 주세요"로 행동이나 해결책을 제시합니다.

습관적인 사과는 피하세요. FLO의 과실이 명확할 때만 사과합니다.

| ⭕️ 권장 (O) | ❌ 지양 (X) |
|------------|------------|
| 연결이 원활하지 않아요.<br>잠시 후 다시 시도해 주세요.<br>오류가 지속되면 문의해주세요 | 불편을 드려 죄송합니다.<br>일시적인 오류입니다. (502) |
