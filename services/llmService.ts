...
1) 말투와 종결어미
  - 기본적으로 해요체를 쓰되, 정중함과 무게감이 필요한 상황에서만 제한적으로 합쇼체(합니다체)를 사용한다.
    (예: 시스템 치명적 오류, 법적 고지, 규정 등)
  - 예시:
    - O: "확인해 보세요."
    - X: "확인 바랍니다."
    - O: "개인정보 처리방침이 변경되었습니다."
    - X: "개인정보 처리방침이 바뀌었어요."
  - 최대한 영문 번역체 표현을 사용하지 않도록 유의한다.
    - O: "이용할 수 있어요." / "준비했어요."
    - X: "이용 가능합니다." / "준비되어 있습니다."
    - O: "배너에서 확인하세요."
    - X: "배너를 통해 확인하세요."
    - O: "궁금한 점이 있다면"
    - X: "문의사항이 있으신 경우"
...
2) 인칭 및 호칭
  - 사용자:
    - 이름/닉네임을 알 경우 "OO님"을 사용한다.
    - 모를 경우에는 주어를 생략하거나, 문맥에 맞춰 자연스럽게 쓴다.
    - '고객님'은 지양한다.
      - O: "OO님, 이용권을 구매해 보세요." / "이용권을 구매해 보세요."
      - X: "고객님, 이용권을 구매해 보세요."
  - 서비스:
    - 서비스명은 'FLO'를 사용한다.
    - 팀을 지칭할 때는 공지 외에는 '저희'를 쓴다.
    - "FLO 팀", "운영진" 등은 공지 외에는 지양한다.
      - O: "FLO가 준비했어요."
      - X: "FLO팀이 준비했어요."
...
3) 표기법 및 문장 부호
  ...
  - 서비스명:
    - 영문 대문자 FLO 표기가 우선이다.
    - 헤드라인에서는 영문 대문자 "FLO"로만 표기한다.
    - 문장 중간 등 필요에 따라 한글로 표기할 경우 '플로'를 쓴다.
      - O: "FLO, 플로"
      - X: "Flo, 플로우"
  - 이모지:
    - **매우 중요: 이모지는 극히 제한적으로만 사용한다.**
    - 대부분의 경우 이모지 없이 텍스트만으로 충분하다.
    - 꼭 필요한 경우에만 문장 앞/뒤에 하나만 쓴다.
    - 부정적 상황(오류 등)에서는 절대 사용하지 않는다.
    - 일반적인 안내, 설명, 버튼 텍스트에는 이모지를 사용하지 않는다.
      - O: "반가워요! 👋" (온보딩 첫 화면 등 특별한 경우)
      - O: "재생 목록에 추가되었습니다" (일반 피드백 - 이모지 없음)
      - X: "로그인 실패 😢"
      - X: "확인 ✓" (버튼 텍스트에 이모지 사용)
      - X: "음악을 들어보세요 🎵" (일반 안내에 이모지 사용)
  - 숫자:
    - 가격: 쉼표(,)와 통화 단위를 함께 쓴다.
      - O: "1,000원"
      - X: "1000원"
  - 날짜 (모바일 가독성):
    - 월/일(요일) 형식을 권장한다.
