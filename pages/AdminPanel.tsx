...
const AdminPanel: React.FC = () => {
    ...
    const handleLogin = () => {
        const adminPassword = import.meta.env.VITE_ADMIN_PASSWORD || '2025PDtone&flo';
                ...
                <div className="bg-white p-8 rounded-xl shadow-lg border border-slate-200 max-w-md w-full text-center">
                    ...
                    <div className="mb-4">
                        <input
                            ...
                            className={`w-full px-4 py-3 rounded-lg border focus:ring-2 focus:ring-indigo-500 outline-none transition ${loginError ? 'border-red-300 focus:border-red-500 bg-red-50' : 'border-slate-300 focus:border-indigo-500'
                                }`}
                            placeholder="비밀번호 입력"
                        />
                        {loginError && (
        ...
        <div className="max-w-6xl mx-auto h-full flex flex-col relative">
            ...
            <div className="flex border-b border-slate-200 mb-6 overflow-x-auto">
                <button
                    ...
                    className={`px-6 py-3 text-sm font-bold flex items-center gap-2 border-b-2 transition-colors whitespace-nowrap ${activeTab === 'GUIDE'
                ...
                <button
                    ...
                    className={`px-6 py-3 text-sm font-bold flex items-center gap-2 border-b-2 transition-colors whitespace-nowrap ${activeTab === 'FEEDBACK'
                            ? 'border-indigo-600 text-indigo-600'
                            : 'border-transparent text-slate-500 hover:text-slate-700'
                        }`}
                >
                                    ...
                                    <div key={item.id} className="p-4 hover:bg-slate-50 transition group">
                                        <div className="flex justify-between items-start mb-2">
                                            <div className="flex items-center gap-2">
                                                <span className={`px-2 py-0.5 rounded text-xs font-bold ${item.type === 'OPINION' ? 'bg-indigo-100 text-indigo-700' : 'bg-green-100 text-green-700'
                                                    }`}>
                                                    {item.type === 'OPINION' ? '서비스 의견' : '사례 제보'}
                                                </span>
